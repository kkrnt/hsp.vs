...
namespace hsp.vs
{
    ...
    [ContentType("hspconf")]
    internal class HSPClassifierProvider : IClassifierProvider
    {
        /// <summary>
        ...
        public IClassifier GetClassifier(ITextBuffer buffer)
        {
            return
                buffer.Properties.GetOrCreateSingletonProperty(
                    delegate { return new HSPClassifier(ClassificationRegistry); });
        }
    }
    ...
    /// </summary>
    internal class HSPClassifier : IClassifier
    {
        ...
        public IList<ClassificationSpan> GetClassificationSpans(SnapshotSpan span)
        {
            ...
            if (text[0] != '#')
            {
                ...
                for (var i = 0; i < split.Length; i++)
                {
                    ...
                    if (HSPData.Keywords.Contains(str.Substring(0, str.Length - 1)))
                    {
                        classifications.Add(
                    ...
                    {
                        if (greensw ||
                            HSPData.Keywords.Count(n => n.IndexOf(str.Substring(0, str.Length - 1)) >= 0) == 1 ||
                            str.Length > 6)
                        {
                            greensw = true;
